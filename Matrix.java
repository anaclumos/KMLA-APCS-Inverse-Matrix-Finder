public class Matrix {
  ...
  public Matrix multiply(Matrix mat) {
    Matrix result = new Matrix(col, mat.rowNum());
    ...
    for (int rowTemp = 0; rowTemp < result.rowNum(); rowTemp++) {
      for (int colTemp = 0; colTemp < result.colNum(); colTemp++) {
        for (int i = 0; i < result.rowNum(); i++) {
          result.changeElement(rowTemp, colTemp,
              result.element(rowTemp, colTemp) + element(rowTemp, i) * mat.element(i, colTemp));
  ...
  public void printMatrix() {
    for (int k = 0; k < row; k++) {
      for (int i = 0; i < col; i++) {
        System.out.print(getFormattedNumber(element[k][i], 3) + "\t");
      }
      System.out.println("");
    }
  }
